#' Provide details about individual events that occur around a constellation
#'  of events
#'
#' A function that reads in multiple time series data frames for various events
#'  and builds variables for each event that occurs within a specified number
#'  of hours within every timestamp.The variables for each event can be
#'  populated with the time the event took place, a dummy variable (0 or 1)
#'  indicating whether or not the eent took place, or the value of the variable
...
#'  variable. Finally, the user selects how to populate the variable built for
#'  each event. 
#'
...
#'  not occur? 2) what is the sequence of events that trigger the combination
#'  of events that I'm interested in? 3) What are the values of each criteria
#'  at the times that each criteria is met?
...
#'  values or can be a linear combination of criteria.
#'
...
#'  value itself stored within the criteria event. The default value is
#'  boolean.
#' @param value_var A string name of the value variable in all data frames.
#'  This argument should only be supplied if the "value" option is selected
...
#' \itemize{
#'  \item missing arguments (only value_var has a default missing value)
...
#'  "PAT_ID", time_var = "RECORDED_TIME", value = "time", value_var = "VALUE")
#'
#' @export

constellate_criteria_2 <- function(..., criteria_names, window_hours, join_key,
  time_var, value = c("boolean", "time", "value"), value_var = NULL) {
  # Build list
  criteria_list <- list(...)
  ...
  # Must pass value_var if select value option
  if (value == "value" & missing(value_var)) {
    stop("Need to specify value_var")
  }

  # If value_var is supplied, ensure it is in all data frames
  if (!missing(value_var)) {
    for (i in seq_len(length(criteria_list))) {
      if (sum(grepl(value_var, names(criteria_list[[i]]))) == 0) {
        stop("'value_var' is not a column name in all time series data frames")
      }
    }
  }
  ...
  for (i in seq_len(length(criteria_list))) {
    ...
    if (value == "time") {
      ...
      # Subset data frames
      criteria_list[[i]] <- data.table(
        criteria_list[[i]][, c(join_key, time_var, value_var), with = FALSE]
      )

      setnames(criteria_list[[i]], value_var, criteria_names[i])
    }
